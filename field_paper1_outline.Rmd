...
Mode share must be designated a tempo-spatial space as same as many aggregated factors. When using mode share, other variables should match the same or upper scales. In contrast, VMT or VHT can reach the individual scale. Moreover, mode share doesn't include the information of driving intensity. Mode share hence has more limitations on AD studies.
...
The same measure could have different scales. Its range, distribution, and meaning could switch along the scales.
Generally, a fine-scale measure contains more information. But if the information is irrelevant to the dependent variables, the study results will not be improved or even be worse.
The urban-form variables' scales could be same or higher than VMT's. 
In disaggregate studies, the former is higher, and these measures are the treatments on each observation.
In aggregate studies, when their scales are the same, the measures are the attributes of each observation. 


    -   Parcel/tax lot

Density at the parcel/tax-lot level measures the net population/built-up density and would return a higher values than that at the higher scales.
Choosing the smaller unit means that there will be more cases having single land use. Entropy, the measure of diversity are more like close to zero.
The local measures like the distance to transit can reflect how easy one accesses transit precisely. For the regional measures such as distance to CBD, the difference among parcels is not very meaningful. 
The parcel or tax-lot scale data could contain built-up information such as floor areas, parking lots, and even property tax. 
Many project-oriented studies like Traffic Impact Analysis use this scale to focus on local issues. But it is hard to extend to other regions.
...
TAZ boundaries are defined by local planners based on long-term observations. Thus TAZ can better reflect the neighborhood travel pattern than the statistical boundaries. The drawback is that there is no easy way to match the census demographic data. 
Similarlly, TAZ refelcts the local urban form rather than the regional urban form.
Like other local data, it should be careful with cross-region comparisons at the TAZ scale.
