...
AD can be measured by driving mode share, distance, and duration. Car ownership can be related to AD somehow, but it doesn't include car use information. The driving frequency itself can not express whether these driving trips are necessary or additional.

    -   VMT

Vehicle miles traveled (VMT) measures the driving distance, often made from travel surveys. Each trip is the basic unit on the survey day and has a VMT value. The observed trips are added to daily VMT per person and VMT per household. The long-term VMT is often estimated from daily VMT. The regional VMT are summarized from individual VMT. Both long-term and regional VMT are not directly observed. In 2017 NHTS data, "trip distance in miles for personally driven vehicle trips" is "derived from route geometry returned by Google Maps API." The VMT from NHTS should be more accurate than that by self-report or straight-line length.
...
Mode share must be designated a tempo-spatial space as same as many aggregated factors. Other variables should match the same or upper scales when using mode share. In contrast, VMT or VHT can reach the individual scale. Moreover, mode share doesn't include the information of driving intensity. Mode share hence has more limitations on AD studies.
...
Urban form means the features of urbanized human settlements or artificial agglomeration. In policy-oriented studies, urban-form features refer to the general characteristics described by some factors such as population, land use, and street network. Each factor could have one or more measures like counts, density, or proportion. That means an urban-form factor and its measures have the same meaning in different regions or urbanized areas. In contrast, the specific features such as Disneyland or Portland Greenways should be analyzed in project-oriented studies.

Urban-form factors can be divided as functional or morphological. This dichotomy can help understand and organize the research framework. Functional factors refer to what people can do in urban areas, such as residing, working, recreation, and travel. Morphological factors mean the distribution and organization of the functions. In gravity law, place mass is related to the functional features, and the morphological features decide distance. In the theory of intervening opportunities, the number of opportunities is determined by the urban functions, and the radium is related to morphology. In utility theory, the functional features benefit through activities, and the morphological features give the costs or impedance.

Demography and transportation system can be broadly interpreted as some parts of the urban form. But AD-urban form studies need to distinguish them from other urban form factors. Demographic and transportation factors are important explanatory variables but is not the targets of land-use planning and policy. A special case, parking belongs both land use and transportation system.

-   Why does it matter?

Urban form reflects some stable features. These features are formed from long-term accumulation. The voluntary change of urban form is a slow, gradual process. The intervention is often costly. If an urban form intensifies the residents' AD, the adverse effects will last many years. Meanwhile, the urban form's influence on AD relief could be invisible and subtle. This effect doesn't require continuous inputs, mandatory regulations, or unfavorable taxation.

-   How is it being measured:

    -   3D/5D/7D

The best known framework is density, diversity, design (3D), distance to transit, destination accessibility (5D), and are further extended to demand management and demographic characteristics (7D). The idea of D-variables is oriented from the concepts of "compact city," "smart growth," and "new urbanism." Urban planners and activists believe a dense, diverse, well-designed city or neighborhood contributes to a sustainable and high-quality urban environment. 

The D-variables framework lists the main influencing factors but doesn't specify their measures.
Density is an intensity factor that can be measured by population density or built-up density - the population size or floor area over the land area. Diversity usually refers to the land-use mix - how many different functions in an area and the composition. Design only includes the street network structure. Because the term design itself means creative and unique. The space qualities are also intricate to quantify. Distance to transit measures the transit service. Destination accessibility can be measured by the distance to a destination or the potential destinations in a given distance. Demand management involves parking space and price as the measures. Demographic characteristics treat the features of human populations as a separate category.

    -   ?

There could be more ways to measure a factor from different dimensions. 
The later studies always tried to add new factors and measures for many years. Currently, The ten measures for 5D-variables are the most widely adopted selection.
...
The same measure could have different scales. Its range, distribution, and meaning could switch along the scales. Generally, a fine-scale measure contains more information. But if the information is irrelevant to the dependent variables, the study results will not be improved or even be worse. The urban-form variables' scales could be same or higher than VMT's. In disaggregate studies, the former is higher, and these measures are the treatments on each observation. In aggregate studies, when their scales are the same, the measures are the attributes of each observation.

    -   Parcel/tax lot

Density at the parcel/tax-lot level measures the net population/built-up density and would return a higher values than that at the higher scales. Choosing the smaller unit means that there will be more cases having single land use. Entropy, the measure of diversity are more like close to zero. The local measures like the distance to transit can reflect how easy one accesses transit precisely. For the regional measures such as distance to CBD, the difference among parcels is not very meaningful. The parcel or tax-lot scale data could contain built-up information such as floor areas, parking lots, and even property tax. Many project-oriented studies like Traffic Impact Analysis use this scale to focus on local issues. But it is hard to extend to other regions.

For the aggregate analysis at this scale, the challenge is how to match the travel survey and census data into the parcels. Another limitation is that the non-residential lots are often the trip destination and do not have a clear AD meaning. For a single-family lot, the household travel behavior can be connected to the lot's attributes. But for the multifamily lot, the studies need to specify the meaning and measures for many households. That means, the studies should divide the parcels or lots into categories.

    -   Census boundaries: bg, ct

One advantage of census boundaries is that it has uniform countrywide criteria and connecting to plenty of census data. Another one is that Block Group and Tracts levels data are randomly sampled and can represent each area's attributes. Block Group and Tract are statistical boundaries. A potential issue is whether they can be treated as an actual entity and have some meanings.

In disaggregate studies, each case locates in a Block Group or Tract. The attributes of this Block Group or Tract are the urban-form treatments on this case. Some measures are not applicable for these census boundaries such as distance-to-transit or buffer-based measures.

Aggregating the travel survey data into the census boundaries is still a problem. The travel survey's sampling design usually cannot reach Census Block Group or Tracts levels. The Local Area Transportation Characteristics for Households (LATCH) by the Bureau of Transportation Statistics (BTS) transfers the NHTS data to the Census tracts level using the household and demographic data from the ACS (<https://www.bts.gov/latch>). But it raises another issue: the estimated tract-level travel variables have been the linear combinations of household and demographic data. They may be not suitable for future regression analysis, including household and demographic variables.

    -   TAZ

TAZ boundaries are defined by local planners based on long-term observations. Thus TAZ can better reflect the neighborhood travel pattern than the statistical boundaries. The drawback is that there is no easy way to match the census demographic data. Similarlly, TAZ refelcts the local urban form rather than the regional urban form. Like other local data, it should be careful with cross-region comparisons at the TAZ scale.

    -   UZA

The boundaries of urbanized areas are defined using uniform criteria by Census. These criteria cut off the rural areas with less population. Since the research topic is about urban form, this cutoff is necessary. UZA boundaries also keep the integrity of urban as a spontaneous entity. It is meaningful since the street network and land use are an organic whole. There is no 'local' measure at the UZA scale. The role of urban-form factors turns from external treatments to intrinsic properties. The individual and neighborhood variances are eliminated. This is an advantage for policy-oriented studies. A drawback of the UZA scale is that many variables are aggregated estimates, such as the average household income or size. They are not directly observed and only reflect a moment of the complete information. For example, among mean and median, one may be the better measure for density and distance.
...
The elasticity for density is small. The elasticity for density in Stevens' results is greater than that in Ewing and Cervero's (2010).
...
The controversy surrounds whether the effect size is large or small, and whether the results can support the idea of compact development. Stevens thinks that the costs of changing urban-form factors should be considered. Otherwise, the effect size is not very useful for practitioners.
Ewing and Cervero argue that less drive has many non-monetary benefits and it is not merely a financial issue.
...
Both single-region and multiple-region data have some limitations. The data in a single region could contain more detailed information. But they can not be extended to other regions and get a more general conclusion. The data from multiple regions, such as different MPO, could have various survey designs, sampling designs, and criteria. It should be careful to use combined data from multiple surveys. Cross-regions data like NHTS have the advantage of collecting all information uniformly. It is more suitable for a cross-sectional study. The drawback is many variables are not available. Moreover, their sampling design may not support applying the data in micro-scale stuides.

-   Research design

D or A: Since 3D variables show a small impact on travel behavior, is it time to replace the framework with a new one? Handy (2017) suggests replacing D-variable with an Accessibility framework. She thinks D-variables affect travel decisions indirectly, but accessibility directly connects to travel behavior. If the change is just about some new variables, the framework has no substantial difference. Suppose accessibility is the critical latent variable. That means the framework should use a dual-layer structure.

Causation or association: Many studies control the residential self-selection to investigate the causal relationship. Some studies try the Propensity Score Matching (PSM) methods. These methods essentially are analyzing a subgroup for a stronger statement. But the policy-oriented studies want a general outcome for the complete population. Moreover, if urban form and travel behavior have no "constant conjunction," this research direction could be fruitless. 
Instead of pursuing the cause, breaking the loop cycle between urban sprawl and AD could be the feasible, useful goal of policymaking, 


-   Methods

Standard regression methods assume the linear relationship between independent and dependent variables.
Some studies have shown that this assumption doesn't hold. Recent studies started to use step function, GAM, or machine learning methods to disclose the non-linear relationships.

The real mechanisms of urban-form impacting travel is an interactive process. The four-step models are also employ an iterative algorithm for matching the flows between origins and destinations. However, most of previous travel-urban form studies get the estimates by one-round computation, which is only one moment of the actual relationship. Some resampling methods like bootstrap or MCMC may improve the analysis.
