...
where $\mathbf{Y}$ are the variable of VMT. $\mathbf{X}$ are all relevant covariates with corresponding coefficients $\boldsymbol{\beta}$. $\boldsymbol{\varepsilon}$ are the error terms.
...
Where $E[\mathbf{Y}|\mathbf{X}]$ is the expected value of individual/household-level VMT $\mathbf{Y}$ conditional on $\mathbf{Y}$; $\mathbf{X}\boldsymbol{\beta}$ is a linear combination (Ditto.); $g$ is a link function.
...
Previous research have identified many factors such as job opportunity, income level, lifecycle stage, habits, and preferences, having stronger impact on travel. Therefore, a sufficient model should contain these strong influencing factors. One purpose is to increase the explanatory power. Another purpose is to control self-selection and other independent variables. 
...
The dichotomy of individual versus environmental factors is a most common framework.
...
Disaggregate analysis usually chooses this structure because the models try to explain the variation of individual travel behavior.
The internal factors, psychological, socio-demographic, and economic characteristics contribute the primary variation in these models.
Take for example VMT models, the model is as follow.

$$\begin{aligned}
\mathbf{Y}=\mathbf{X}_\mathrm{C}\boldsymbol{\beta}_\mathrm{C}+\mathbf{X}_\mathrm{E}\boldsymbol{\beta}_\mathrm{E}+\boldsymbol{\varepsilon}
\end{aligned}$$

where $\mathbf{X}_\mathrm{C}$ travelers' internal characteristics; $\mathbf{X}_\mathrm{E}$ are built environment and other environment covariates.
